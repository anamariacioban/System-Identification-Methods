...
\[
K = \frac{\overline{y}_{st} - \overline{y}_{0}}{\overline{u}_{st} - \overline{u}_{0}}
\]
...
The dominant time constant is then computed from the slope \( \alpha \):
...
The inflection time is estimated based on the indices:
- \( i_7 \): step application moment;
- \( i_8 \): inflection point of the response.
